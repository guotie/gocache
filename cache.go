...
var (
	...
	ErrCacheMiss  = errors.New("gocache: key not found.")
	ErrNotStored  = errors.New("gocache: not stored.")
	ErrNotSupport = errors.New("gocache: not support this driver.")
	ErrDrvExist   = errors.New("gocache: driver has registered.")

	adapters map[string]Cache = make(map[string]Cache)
)
...
func Register(driver string, empty_ins Cache) error {
	_, ok := adapters[driver]
	if ok {
		return ErrDrvExist
	}
	adapters[driver] = empty_ins
	return nil
}

func Open(driver, config string) (err error) {
	ad, ok := adapters[driver]
	if !ok {
		return ErrNotSupport
	}

	Instance, err = ad.Open(config)
	if err != nil {
		return err
	}
	return nil
